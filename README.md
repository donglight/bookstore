...
## 目的与缺陷
  ...
  - 缺陷: 购物车和订单不能分店铺统计和付款；前端界面较为简陋，许多地方的用户体验性也不好；富文本编辑器没有实现文件上传的功能；后台API并没有完全符合     restful风格等等。总的来说，功能较为简单，许多功能还没有完成。
## 依赖环境
  - jdk1.8,maven,mysql
  - 注意事项
...
## 系统划分与功能
  - 该系统分为前台展示和后台管理两大模块。  
  - 前台主要是为消费者服务。该子系统实现了注册，登录，以及从浏览、下单到支付的整个流程，支付使用的是支付宝的沙箱环境(支付账号:ynkltg9762@sandbox.com,密码:111111)，属于模拟环境。需要注册沙箱账号才能付款。  
